            ...
            for (let h = 0; h < beltNum; ++h) {
                ...
                for (let i = 0; i <= div; ++i) { // まんなか
                    for (let j = 0; j <= div; ++j) {
                        ...
                        v.r0 = [0, by, capsuleR * scale]; // +Z は奥
                        v.r1 = [0, by - beltHeight, -capsuleR * scale]; // -Z は手前
                        v.c = [0, y, 0];
 ...
 */
            ...
            r.moveDamping = moveDamp;
            r.rotDamping = rotDamp;
...
// even が tree
            ...
            default:
                ...
                r.friction = 1000;
                r.mass = 0.002; // 重量
                r.setUINots(RIGID_DEFAULT_GROUP); // 自分
//                r.setUINots(1, 2, 5);
                if ((i & 1) !== 0) { // odd がエフェクト
                    j = null;
                    if (i !== baseBoneIndex) {
...
// 半径、不使用、不使用
                r.size = [capsuleR * scale, scale, scale];
                r.p = [...b.p];
                break;
            }
