...
export class TexMaker {
   ...
   * @param {number} x 
    */
  srgbtolinear(x) {
    //return Math.sqrt(x); // 1/2.2どころか1/2

    return Math.max(0, (x ** 0.416666667) * 1.055 - 0.055); // 上だけ

    // 区間分割でちゃんとする場合
    const low = x * 12.92;
    const high = (x ** (1 / 2.4)) * 1.055 - 0.055;
    return (x < 0.0031308) ? low : high;
  }
...
{ cx: 1/8, cy: 0.75, deg: -1, top: 1, k: 8, bx: 0.5, by: 8, add: true },
   ...
   * @param {number} index 8x8で0～63のどこか
   */
  drawOwnBone(canvas, index, param) {
    const padding = 4;
    const whole = canvas.width;
    const side = whole / 8;
    const bx = (index % 8) * side;
...

    {
      ...
      /** @type {number[]} */
      let cola = param.cola || [238, 238, 204];

      let col = param.col || [204, 204, 204];
      /** @type {number[]} */
      let colb = param.colb || [170, 170, 204];
